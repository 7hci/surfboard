...
trello.createTrelloBoard = (contractor, socket, credentials) => trello.addBoard(contractor)
    .then(boardId => Bluebird.all(
        Bluebird.map(config.get('trello.team.members'), (member) => {
        ...
        trello.addList(boardId, 'Done')
          ...
          .then(() => Bluebird.all([trello.addList(boardId, 'To Do'), drive.getTasksFromFile(credentials)])
              .spread((listId, tasks) => Bluebird.mapSeries(tasks, (task) => {
    ...
    .then(() => {
      ...
      socket.emit('update', { text: 'Created board on Trello', status: 'success', task: 'createTrelloBoard' });
      return { text: 'Created board on Trello', status: 'success' };
    }
    )
    .catch((err) => {
      logger.error(err);
      socket.emit('update', { text: 'Problem creating board on Trello', status: 'failure', task: 'createTrelloBoard' });
