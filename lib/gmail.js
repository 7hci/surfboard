...
gmail.sendWelcomeEmail = (contractor, socket, credentials) => googleAuth.getAccessToken(credentials)
  ...
  .then((messageData) => {
    if ('id' in messageData) {
      ...
      socket.emit('update', { text: 'Sent welcome e-mail', status: 'success' });
    } else {
      socket.emit('update', { text: 'Problem sending welcome e-mail', status: 'failure' });
    }
  })
  .catch((err) => {
    logger.error(err);
...
gmail.sendLoginEmail = (contractor, socket, credentials) => googleAuth.getAccessToken(credentials)
  ...
  .then((messageData) => {
    if ('id' in messageData) {
      ...
      socket.emit('update', { text: 'Sent e-mail with credentials', status: 'success' });
    } else {
      socket.emit('update', { text: 'Problem sending e-mail with credentials', status: 'failure' });
    }
  })
  .catch((err) => {
    logger.error(err);
 ...
 * @param socket
 * @param formArray serialized form information containing e-mail details
 * @param newHire object with new hire info saved to database
 * @param credentials Google auth credentials stored in the user's sessions
 * @returns the encoded e-mail
 */
gmail.sendMSSA = (socket, formArray, newHire, credentials) => {
  ...
  const message =
    `${formData.message}\n\nhttp://surfboard.reardenapps.com/talent/${newHire.id}`;
  const cc = formData.cc;
  let person;

  return Bluebird.all([
    googleAuth.getAccessToken(credentials),
    models.NewHire.findById(newHire.id)
  ])
    .then((result) => {
