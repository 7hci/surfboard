...
exports.addUserToClickTime = (contractor, socket) => {
  let command = `casperjs dist/server/lib/clicktime-casper.js --user=${config.get('clicktime.user')
  ...
  return new Bluebird((resolve) => {
    shell.exec(command, (code, stdout, stderr) => {
      if (stderr) {
        logger.error(stderr);
        socket.emit('update',
          { text: 'Problem adding user to ClickTime', status: 'failure', task: 'addUserToClickTime' });
      } else {
        try {
