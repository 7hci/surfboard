 ...
 */
clicktime.addUserToClickTime = (contractor, socket) => {
  let command = `casperjs dist/lib/clicktime-casper.js --user=${config.get('clicktime.user')
  ...
  return new Promise((resolve) => {
    shell.exec(command, (code, stdout, stderr) => {
      if (stderr) {
        logger.error(stderr);
        socket.emit('update', { text: 'Problem adding user to ClickTime', status: 'failure' });
      } else {
