 ...
 */
drive.addAndShareDriveFolder = (contractor, socket, credentials) => {
  drive.createFolder(contractor, credentials)
    ...
    .then(() => {
      ...
      socket.emit('update', { text: 'Created and shared Drive folder', status: 'success' });
    })
    .catch((err) => {
      logger.error(err);
      socket.emit('update', { text: 'Problem creating and sharing Drive folder', status: 'failure' });
    });
};

/**
 * Helper function to create a folder if one doesn't already exist
 * @param contractor
 * @param credentials Google auth credentials stored in the user's sessions
 * @returns folder id
...
drive.createFolder = (contractor, credentials) => {
  ...
  if (contractor.folderId) {
    return Bluebird.resolve(contractor.folderId);
  }
...
/**
 * Helper function to get a file from Drive as a pdf
 * @param credentials Google auth credentials stored in the user's sessions
 * @param fileId the id of the requested file
 * @returns an array of CSV strings representing the tasks
 * @throws error if the tasks could not be retrieved
 */
drive.getPdfFromDrive = (credentials, fileId) => {
  const driveUrl = `${config.get('google.baseUrl')}/drive/v3/files/${fileId}/export`;
